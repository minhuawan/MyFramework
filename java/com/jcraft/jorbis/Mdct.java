...
/*     */     int i;
/*  53 */     for (i = 0; i < n / 4; i++) {
/*  54 */       this.trig[AE + i * 2] = (float)Math.cos(Math.PI / n * (4 * i));
/*  55 */       this.trig[AO + i * 2] = (float)-Math.sin(Math.PI / n * (4 * i));
/*  56 */       this.trig[BE + i * 2] = (float)Math.cos(Math.PI / (2 * n) * (2 * i + 1));
/*  57 */       this.trig[BO + i * 2] = (float)Math.sin(Math.PI / (2 * n) * (2 * i + 1));
/*     */     } 
/*  59 */     for (i = 0; i < n / 8; i++) {
/*  60 */       this.trig[CE + i * 2] = (float)Math.cos(Math.PI / n * (4 * i + 2));
/*  61 */       this.trig[CO + i * 2] = (float)-Math.sin(Math.PI / n * (4 * i + 2));
/*     */     } 
/*     */ 
/*     */     
/*  65 */     int mask = (1 << this.log2n - 1) - 1;
/*  66 */     int msb = 1 << this.log2n - 2;
/*  67 */     for (int j = 0; j < n / 8; j++) {
/*  68 */       int acc = 0;
/*  69 */       for (int k = 0; msb >>> k != 0; k++) {
/*  70 */         if ((msb >>> k & j) != 0)
/*  71 */           acc |= 1 << k; 
/*  72 */       }  this.bitrev[j * 2] = (acc ^ 0xFFFFFFFF) & mask;
/*     */       
/*  74 */       this.bitrev[j * 2 + 1] = acc;
/*     */     } 
/*     */     
...
/*     */       
/* 169 */       float f1 = x[xA] - x[xB];
/* 170 */       A -= 4;
/*     */       
/* 172 */       w[i++] = x0 * this.trig[A] + f1 * this.trig[A + 1];
/* 173 */       w[i] = f1 * this.trig[A] - x0 * this.trig[A + 1];
/*     */       
...
/* Location:              D:\files\slayTheSpire-export\!\com\jcraft\jorbis\Mdct.class
